...
;; test-evaluate : scheme-expression list-of-functions number -> test-result
;; Test for evaluate-with-defs
(define (test-evaluate expr lof expected-result)
  (cond
  [(equal? (evaluate-with-defs expr lof) expected-result) true]
  [else (list "Bad application" expr lof expected-result)]))


;; Checks for evaluate-with-defs
(test-evaluate (make-app 'square (make-app 'triple 3)) (list double square triple) 81)
(test-evaluate (make-app 'triple (make-app 'square 2)) (list square double triple) 124)
