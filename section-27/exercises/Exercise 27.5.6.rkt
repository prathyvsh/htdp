...
;; Cannot proceed with elimination since
;; Two rows simultaneously becomes 0.
...
(define (triangulate lon)
  (cond
   [(empty? lon) empty]
   [(and (zero? (first (first lon))) (zero? (first (second lon)))) (error 'triangulate "This system of equations can't be solved")]
