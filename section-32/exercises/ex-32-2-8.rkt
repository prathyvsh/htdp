...
;; Caution: The function only produces a solution in case the board is solvable.
(define (mc-solution los)
  (local ((define end (final-states los))
          (define (mc-solution-acc los traversed-boards)
            (cond
             [(empty? los) false]
             [(cons? end) (board-trails end)]
             [else (mc-solution-acc (all-possible-boards (difference los traversed-boards)
                                                         (cons los traversed-boards))]))))
  (mc-solution empty))
...
;; TESTS
(define start (make-board (make-distribution 3 3) (make-boat true (make-distribution 0 0)) (make-distribution 0 0) empty))
(define end (make-board (make-distribution 0 0) (make-boat false (make-distribution 0 0)) (make-distribution 3 3) empty))
(define one-before-last (make-board (make-distribution 0 1) (make-boat true (make-distribution 0 0)) (make-distribution 2 3) empty))

(draw-boards (first (mc-solution (list start))))
