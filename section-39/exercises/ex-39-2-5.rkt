...
(define (moving-pictures init-posn p)
  (local (
          ;; picture : (listof shape)
          ;; Keeps track of the picture
          (define pic p)
          
          ;; last-posn : posn
          ;; Keeps track of the last position
          (define last-posn init-posn)

          ;; update-pic -> picture
          ;; Effect moves the picture according to the location
          (define (update-pic new-posn)
            (begin (set! last-posn new-posn)
                   (translate-picture-lr (posn-x new-posn)
                                         (translate-picture-ud (posn-y new-posn) pic))))
          
          ;; init -> void
          ;; Effect : Draws the picture on screen
          (define (init)
            (draw-losh pic))
          
          ;; measure-shift : posn -> posn
          ;; Calculates the displacement between two posns
          (define (measure-shift a b)
            (make-posn (- (posn-x a) (posn-x b)) (- (posn-y a) (posn-y b))))
          ...
          (define (translate-picture-lr delta pic)
            (translate-losh translate-shape-lr delta pic))

          ;; translate-picture-ud : number (listof shape) -> (listof shape)
          ;; Translates a list of given shape by delta amount vertically
          (define (translate-picture-ud delta pic)
            (translate-losh translate-shape-ud delta pic))
          ...
          ;; place : posn ->(void)
          ;; Places the picture at a given position
          (define (place new-posn)
            (begin (clear-losh (update-pic last-posn)) (draw-losh (update-pic new-posn))))
          
          ;; reset -> (void)
          ;; Effect: Resets the location of the
          ;; object to the initial position
          (define (reset)
            (begin (clear-losh (update-pic last-posn)) (draw-losh (update-pic init-posn))))
          
          
          ;; service-manager : sym -> service
          (define (service-manager msg)
            (cond
             [(symbol=? msg 'place) place]
             [(symbol=? msg 'reset) (reset)]
             [(symbol=? msg 'last) last-posn]
...
(define mover (moving-pictures (make-posn 0 0) LUNAR))
((mover 'place) (make-posn 30 30))
(mover 'last)
((mover 'place) (make-posn 50 50))
(mover 'last)
(mover 'reset)

;; NOTE TO READER
;; The solution is slightly contrived due to the reuse of
;; translate function from the previous exercises. Replacement
;; of current translate functions with a place function would
;; better fit the specifications. But I chose to follow this
;; route for i) reuse the components ii) to practice how
;; much contrivance would have to be suffered if you use
;; different verbs/actions too achieve your purpose.
