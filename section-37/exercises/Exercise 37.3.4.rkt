...
;; chosen-word : word
;; the word that the player is to guess
(define letters-remaining (length chosen-word))
...
(define (hangman)
  (begin
    ...
    (set! letters-remaining (length chosen-word))
  ...
  (local ((define (reveal-one chosen-letter status-letter)
	    (cond
		 ...
		 (set! new-knowledge true)
                 (set! letters-remaining (sub1 letters-remaining))
		 guess)]
	      [else status-letter])))
    (begin
      (set! new-knowledge false)
