...
;; shape-to-move : (listof shape)
;; Tracks the shape to be moved
(define shape-to-move LUNAR)

;; fuel : number
;; Keeps track of the remaining fuel
(define fuel 100)

;; VIEW
(define up (make-button "^" (lambda (e) (move-control 'up))))
(define down (make-button "V" (lambda (e) (move-control 'down))))
(define left (make-button "<" (lambda (e) (move-control 'left))))
(define right (make-button ">" (lambda (e) (move-control 'right))))

(define move-by-field (make-text "Pixels to move by:"))
(define status (make-message "Start by entering an amount of pixels"))

(define fuel-label (make-message (number->string fuel)))

;; CONTROLLER
;; move-control : symbol -> void
;; Moves the shape in the given direction
(define (move-control direction)
  (begin (local ((define delta (string->number (text-contents move-by-field))))
           (if (number? delta)
               (cond
                [(equal? direction 'left) (transport (- delta) move-picture-lr)]
                [(equal? direction 'right) (transport delta move-picture-lr)]
                [(equal? direction 'up) (transport (- delta) move-picture-ud)]
                [(equal? direction 'down) (transport delta move-picture-ud)]
                [else (error 'move-control "Unknown direction")])
               (draw-message status "Please enter a number")))
         (draw-losh shape-to-move)))

;; transport : number (delta picture -> true or picture) -> void
;; Moves the shape by the given delta amount in the given direction
(define (transport delta direction-fn)
  (begin (set! fuel (- fuel (abs delta)))
         (set! shape-to-move (direction-fn delta shape-to-move))
         (draw-message fuel-label (number->string fuel))))
...
(define (drive)
  (begin (start 300 300)
         (draw-losh shape-to-move)
         (create-window
          (list (list up down left right)
                (list move-by-field)
                (list status fuel-label)))))
