...
(define (check-choice e)
  ...
  (local ((define current-choice (list-ref ALPHABETS (choice-index guess-maker))))
    (cond
     ...
     [else (local ((define result (hangman-guess (string->symbol current-choice)))) (begin (map pretty-print (list current-choice result)) (set! guesses-so-far (cons current-choice guesses-so-far))
                  (cond
