...
;; how-many-ancestors : ftn -> number
;; Counts the number of ancestors a given node has.
(define (how-many-ancestors ft)
  (cond
   [(or (false? (person-father ft)) (false? (person-mother ft))) 1]
   [(false? (person-father ft)) (+ 1 (how-many-ancestors (person-mother ft)))]
   [(false? (person-mother ft)) (+ 1 (how-many-ancestors (person-father ft)))]
   [else (+ 1 (how-many-ancestors (person-father ft)) (how-many-ancestors (person-mother ft)))]))
...
(equal? (how-many-ancestors gustav) 5)
(equal? (how-many-ancestors adam) 3)
(equal? (how-many-ancestors fred) 1)
